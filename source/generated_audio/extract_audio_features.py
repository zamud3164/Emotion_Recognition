...
#audio_vectors_path = 'source/generated_audio/audio_vectors_generated_audio.csv'
#audio_vectors_file = pd.read_csv(audio_vectors_path)
#audio_vectors = audio_vectors_file['audio_vector']

#data_dir = 'data/'
df_data_path = 'source/generated_audio/data/mosei_test_filtered.csv'
audio_vectors_path = 'source/generated_audio/data/audio_vectors_gan.pkl'

#audio_vectors_file['audio_vector'] = audio_vectors_file['audio_vector'].apply(lambda x: json.loads(x))
...
for index, row in tqdm(df_data.iterrows()):
    #wav_file_name = audio_vectors_file['wav_file'][index]
    #label = emotion_dict[audio_vectors_file['label'][index]]
    #y = audio_vectors_file['audio_vector'][index]
    #y = np.array(y)
...
df_features.to_csv('source/generated_audio/data/audio_features_gan_v0.csv', index=False)
